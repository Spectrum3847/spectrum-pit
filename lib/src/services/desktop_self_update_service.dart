...
class DesktopSelfUpdateService {
  ...
  Future<void> _updateWindowsZip(Uri url, String expectedSha256) async {
    ...
    final response = await _downloadVerified(url, expectedSha256);
    final staging = Directory.systemTemp.createTempSync('spectrum_pit_update_');
    try {
  ...
  static String windowsRelaunchScript({
    ...
    final lines = <String>[
      '@echo off',
      'rem Spectrum Pit updater: written by the app next to the staged',
  ...
  static String macosRelaunchScript({
    ...
    final lines = <String>[
      '#!/bin/sh',
      '# Spectrum Pit updater: written by the app next to the staged',
