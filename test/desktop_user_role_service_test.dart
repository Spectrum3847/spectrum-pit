...
import 'package:spectrumpit/src/services/desktop_user_role_service.dart';
    ...
    test('reads an existing profile', () async {
      final service = DesktopUserRoleService(
        firestore: _firestore(
          MockClient(
            (_) async => http.Response(_profileDoc('u1', ['pit']), 200),
          ),
        ),
      );
    ...
    final service = DesktopUserRoleService(
      firestore: _firestore(
        MockClient((request) async {
          ...
          return http.Response(_profileDoc('u2', ['pit']), 200);
        }),
      ),
    );
    await service.updateRoles('u2', {UserRole.pit});
