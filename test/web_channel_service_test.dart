...
import 'package:spectrumpit/src/services/web_channel_service.dart';

void main() {
  group('channelForHost', () {
    test('maps each hosting site and its firebaseapp.com twin', () {
      expect(
        WebChannelService.channelForHost('spectrumpit.web.app'),
        WebChannel.stable,
      );
      expect(
        WebChannelService.channelForHost('spectrumpit.firebaseapp.com'),
        WebChannel.stable,
      );
      expect(
        WebChannelService.channelForHost('spectrumpit-staging.web.app'),
        WebChannel.staging,
      );
      expect(
        WebChannelService.channelForHost('spectrumpit-staging.firebaseapp.com'),
        WebChannel.staging,
      );
    });
    ...
    test('is null for preview hosts, localhost, and anything else', () {
      expect(
        WebChannelService.channelForHost('spectrumpit-preview.web.app'),
        isNull,
      );
  ...
  group('hasPublishedBuild', () {
    ...
    test('is true when the site serves a version.json', () async {
      ...
      expect(
        requested,
        Uri.parse('https://spectrumpit-staging.web.app/version.json'),
      );
    });
