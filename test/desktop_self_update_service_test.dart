...
void main() {
  ...
  group('windows zip update (#423)', () {
    ...
    setUp(() {
      ...
      addTearDown(() => dir.deleteSync(recursive: true));
      installDir = '${dir.path}/install/SpectrumPit';
      Directory(installDir).createSync(recursive: true);
      exePath = '$installDir/spectrumpit.exe';
      ...
      () async {
        ...
        final svc = DesktopSelfUpdateService(
          ...
          extractArchive: (archive, destinationPath) async {
            ...
            File('$destinationPath/spectrumpit.exe').writeAsBytesSync([1]);
          },
          makeExecutable: (_) async {},
          relaunch: (p) async => relaunched = p,
          platformLoader: () => DesktopSelfUpdatePlatform.windows,
  ...
  group('macos app bundle update (#423)', () {
    late Directory dir;
    ...
    test(
      ...
      () async {
        ...
        final svc = DesktopSelfUpdateService(
          ...
          extractArchive: (_, destinationPath) async {
            stagedApp = '$destinationPath/Spectrum Pit.app';
            Directory('$stagedApp/Contents/MacOS').createSync(recursive: true);
            File('$stagedApp/Contents/MacOS/spectrumpit').writeAsBytesSync([1]);
          },
          makeExecutable: (_) async {},
          relaunch: (p) async => relaunched = p,
    ...
    test('throws when the extracted bundle lacks the executable', () async {
      ...
      final svc = DesktopSelfUpdateService(
        ...
        extractArchive: (_, destinationPath) async {
          stagingDir = Directory(destinationPath).parent.path;
          Directory('$destinationPath/SpectrumPit.app/Contents')
    ...
    test('throws before downloading when not inside an .app bundle', () async {
      ...
      final svc = DesktopSelfUpdateService(
        ...
        runningExePathLoader: () => '/opt/bin/spectrumpit',
  ...
  test('appBundleContaining walks up to the enclosing bundle', () {
    expect(
      DesktopSelfUpdateService.appBundleContaining(
        '/Applications/Spectrum Pit.app/Contents/MacOS/spectrumpit',
      ),
      '/Applications/Spectrum Pit.app',
    );
    expect(
  ...
  test('windowsRelaunchScript swaps via siblings and rolls back', () {
    final script = DesktopSelfUpdateService.windowsRelaunchScript(
      ...
      installDir: r'C:\Program Files\SpectrumPit',
      executableName: 'spectrumpit.exe',
    );
    ...
    expect(script, contains(r'set "INSTALL=C:\Program Files\SpectrumPit"'));
  ...
  test('macosRelaunchScript swaps via siblings and restores on failure', () {
    final script = DesktopSelfUpdateService.macosRelaunchScript(
      ...
      pid: 99,
      stagedAppBundle: '/var/folders/x/T/stage/Payload/SpectrumPit.app',
      installedAppBundle: '/Applications/Spectrum Pit.app',
    );
    ...
    expect(
      script,
      contains("STAGED='/var/folders/x/T/stage/Payload/SpectrumPit.app'"),
    );
    expect(script, contains("INSTALL='/Applications/Spectrum Pit.app'"));
  ...
  test('macosRelaunchScript escapes single quotes in paths', () {
    final script = DesktopSelfUpdateService.macosRelaunchScript(
      stagingDir: '/tmp/staging',
      pid: 7,
      stagedAppBundle: "/tmp/O'Brien stage/SpectrumPit.app",
      installedAppBundle: "/Applications/O'Brien.app",
    );

    expect(script, contains("STAGED='/tmp/O'\\''Brien stage/SpectrumPit.app'"));
    expect(script, contains("INSTALL='/Applications/O'\\''Brien.app'"));

    expect(script, isNot(contains("/Applications/O'B")));
  });

  group('macosRelaunchScript executes against a real sh (#436)', () {
    late Directory dir;
    ...
    setUp(() {
      ...
      installBundle = '${dir.path}/Applications/Spectrum Pit.app';
      Directory('$installBundle/Contents/MacOS').createSync(recursive: true);
      installedExe = File('$installBundle/Contents/MacOS/spectrumpit')
        ..writeAsStringSync('old');
      stagedBundle = '${dir.path}/stage/Spectrum Pit.app';
      Directory('$stagedBundle/Contents/MacOS').createSync(recursive: true);
      File('$stagedBundle/Contents/MacOS/spectrumpit').writeAsStringSync('new');
    ...
    test('a failed restore opens the old bundle where it sits', () async {
      ...
      final parkedExe = File(
        '$installBundle.update-old/Contents/MacOS/spectrumpit',
      );
  ...
  test('shellQuote wraps paths and escapes embedded quotes', () {
    expect(
      DesktopSelfUpdateService.shellQuote('/Applications/SpectrumPit.app'),
      "'/Applications/SpectrumPit.app'",
    );
    expect(
