...
const CACHE = 'spectrum-pit-{{BUILD_ID}}';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        names
          .filter((name) => name.startsWith('spectrum-pit-') && name !== CACHE)
...
async function shellResponse(event, request) {
  ...
  try {
    ...
    return new Response(
      'Spectrum Pit is offline and has not been loaded on this device yet.',
