#!/usr/bin/env python3
"""Generate an AltStore source JSON for a Spectrum Pit distribution channel.
...
APP_NAME = "Spectrum Pit"
SUBTITLE = "FRC pit logistics"
...
BUNDLE_ID = "org.spectrum3847.spectrumpit"
DEVELOPER_NAME = "Spectrum 3847"
TINT_COLOR = "3C0060"
CATEGORY = "utilities"
APP_DESCRIPTION = "Pit inventory, packing, and borrowed tool tracking for FRC teams."

APP_PERMISSIONS = {
    "entitlements": [],
    "privacy": {
        "NSCameraUsageDescription": "Used to photograph how a tool was packed, so "
        "the crew unpacking it can see what it should look like.",
        "NSPhotoLibraryUsageDescription": "Used to attach an existing photo of a "
        "packed item to its packing record.",
    },
}
...
RETIRED_RELEASE_HOST = "Spectrum3847/spectrum-pit-releases"
CURRENT_RELEASE_HOST = "Spectrum3847/spectrum-pit"
...
def _warn_suspicious_version(version):
    """Heuristic guard for the #136/#789 class of bug.
    ...
    Xcode actually baked in, which mismatches the IPA and breaks plain
    AltStore installs. Warn only -- the caller knows whether a hyphen is
    legitimate.
    """
    if "-" in version:
        print(
            f"Warning: --version '{version}' contains a '-' which usually "
            "means it was not read back from the built Info.plist (see #136).",
            file=sys.stderr,
        )
