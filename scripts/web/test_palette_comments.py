#!/usr/bin/env python3
"""Every PitPalette token named in web/ must match the palette.
...
by hand in one sitting, two of them, which is why this
...
PALETTE = Path("lib/src/theme/pit_palette.dart")
WEB_FILES = ("web/index.html", "web/offline-check.html")

CLAIM = re.compile(r"#([0-9A-Fa-f]{6})'?[;,]?\s*/\*\s*PitPalette\.(\w+)")
            ...
            for value, token in claims(path):
                with self.subTest(path=path, token=token):
                    self.assertIn(
                        token,
                        palette,
                        f"{path} names PitPalette.{token}, which does not exist",
                    )
                    self.assertEqual(
                        palette[token],
                        value,
                        f"{path} says #{value} is PitPalette.{token}, "
